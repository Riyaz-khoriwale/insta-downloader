...
const VideoPreview: React.FC<VideoPreviewProps> = ({ video }) => {
  ...
  return (
    <div className="w-full max-w-3xl mt-12 bg-white/60 backdrop-blur-sm border border-gray-200 rounded-2xl shadow-xl overflow-hidden p-4 sm:p-8 animate-fadeInUp">
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="md:w-2/3 flex flex-col justify-center">
          <h3 className="text-xl lg:text-2xl font-bold text-gray-900 mb-2 line-clamp-2">{video.title}</h3>
          <p className="text-gray-500 mb-6">by <span className="font-semibold text-gray-700">@{video.author}</span></p>

          <fieldset className="mb-6">
            <legend className="text-gray-800 font-semibold mb-3">Select Quality for Download:</legend>
            <div className="space-y-3">
              {video.downloadOptions.map((option) => {
                const isCopied = copiedUrl === option.url;
                return (
                  <div key={option.quality} className="flex items-center justify-between gap-3 p-3 rounded-lg bg-gray-100 border-2 border-transparent has-[:checked]:border-pink-500 has-[:checked]:bg-pink-500/10 transition-all duration-200">
                    <div className="flex items-center gap-3">
                        <input
                            ...
                            className="h-4 w-4 accent-pink-500 bg-gray-200 border-gray-400 focus:ring-pink-500 cursor-pointer"
                        />
                        <label htmlFor={`quality-${option.quality}`} className="cursor-pointer flex flex-col sm:flex-row sm:items-baseline">
                            <span className="font-bold text-gray-900">{option.quality}</span>
                            <span className="text-gray-500 text-sm sm:ml-2">({option.size})</span>
                        </label>
                    </div>
                    <button
                        onClick={() => handleCopyLink(option.url)}
                        disabled={isCopied}
                        className={`flex-shrink-0 flex items-center justify-center gap-2 text-sm font-semibold py-1 px-3 rounded-md transition-all duration-200 disabled:cursor-not-allowed ${isCopied ? 'bg-green-100 text-green-600 w-28' : 'bg-gray-200 hover:bg-gray-300 text-gray-700 w-28'}`}
                    >
                        {isCopied ? (
                        <>
          ...
          <div className="flex flex-col items-center gap-3 h-12">
            ...
            {downloadState === 'complete' && (
              <div className="w-full flex items-center justify-center gap-2 text-green-500 font-semibold text-lg">
